...
let wordAsArray (word:string) =

   let wordAsArray = word.ToCharArray()
   let wordLength  = word.Length

   {|word=word;wordAsArray=wordAsArray;wordLength=wordLength|}

let readThroughLetters (word_info:{|word:string;wordAsArray:char array;wordLength:int|}) =
    ...
    seq { for i = 0 to word_info.wordLength - 1 do
              ...
              match found with
              | true  -> yield {|wordAsArray=word_info.wordAsArray;wordLength=word_info.wordLength;res=Some res1;letterPosition=i|}
              | false -> yield {|wordAsArray=word_info.wordAsArray;wordLength=word_info.wordLength;res=None;letterPosition=i|}

        }

let forEachLetterReturnCandidateXY (word_info:{|wordAsArray:char array;wordLength:int;res:Coordinate list;letterPosition:int|} )=

    let letterPOSITION = i + 1
    let wordsplit = {offsetOfIntersectingLetter=i; positionOfIntersectingLetter=letterPOSITION; NumberOflettersBeforeTheIntersectionLetter=letterPOSITION - 1; NumberOflettersAfterTheIntersectionLetter=wordLength - letterPOSITION}
    let candidate_coordinate_and_letter_dict_index = res1 |> Seq.mapi(fun i xy -> (i, xy) )
    let xx = seq { for (letter_dict_index,xy) in candidate_coordinate_and_letter_dict_index do 
...
let limit_matching_XY_per_letter (word_count:int, word:string) =
    ...
    seq { let candidateXY =   
             ...
             |> wordAsArray
             |> readThroughLetters
             |> forEachLetterReturnCandidateXY
